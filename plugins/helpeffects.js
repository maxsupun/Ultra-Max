...
`⚙️විධානය: *.vintage*\nℹ️විස්තර: එය සපයා ඇති textයේ buld display රූපයක් යවයි. \n_(Ex: .vintage Ultra/Max_\n\n` + 
`⚙️විධානය: *.bplogo*\nℹ️විස්තර: එය සපයා ඇති textයේ Supun pink logo යවයි.\n\n` + 
...
`⚙️Command: *.bplogo*\nℹ️Description: Text to Supun Pink themed logo.\n\n` + 
...
if (Config.LANG == 'EN') {
    if (Config.WORKTYPE == 'private') {
        Max.applyCMD({pattern: 'effectimg', fromMe: true,  deleteCommand: false, desc: Lang.EFFECT_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }

    else if (Config.WORKTYPE == 'public') {
        Max.applyCMD({pattern: 'effectimg', fromMe: false, desc: Lang.EFFECT_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }
}
