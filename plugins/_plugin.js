...
Max.applyCMD({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, dontAddCommandList: true, deleteCommand: false,}, (async (message, match) => {
    if (match[1] === '') return await message.sendMessage(Lang.NEED_URL + '.install https://gist.github.com/maxsupun/49afd28b1932095cc76facbcedef3482')
    try {
...
Max.applyCMD({pattern: 'plugin', fromMe: true, desc: Lang.PLUGIN_DESC, deleteCommand: false,dontAddCommandList: true}, (async (message, match) => {
...
Max.applyCMD({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC, dontAddCommandList: true}, (async (message, match) => {
