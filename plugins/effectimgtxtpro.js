/* Copyright (C) 2021 Ultra Max.

Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.

Ultra Max - Supun Max - Ultra Max
*/

const Max = require('../events');
const Config = require('../config');
const {MessageType, MessageOptions, Mimetype} = require('@adiwajshing/baileys');
const fs = require('fs');
const os = require('os');
const qatxtpro = require('UltraMax-textlogo-1.0.3'); 
const axios = require('axios');
const request = require('request');
var code = { cd: 'L3Jvb3QvUXVlZW5BbWRpLw==', pay: '' }    
var bufcode = Buffer.from(code.cd, 'base64')
var utfbuff = bufcode.toString('utf-8')
code.pay = utfbuff
if (os.userInfo().homedir !== code.pay) return;

// LIST
const Language = require('../language');
const Lang = Language.getString('effects');
let LOL = Config.WORKTYPE == 'public' ? false : true

Max.applyCMD({pattern: 'demon ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-green-horror-style-text-effect-online-1036.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/demon2.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/demon2.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'rust ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-metallic-text-effect-free-online-1041.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/rust.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/rust.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'transformer ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-transformer-text-effect-online-1035.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/transformer.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/transformer.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'fiction ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-science-fiction-text-effect-online-free-1038.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/fiction.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/fiction.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'berry ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-berry-text-effect-online-free-1033.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/berry.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/berry.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'thunder ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/online-thunder-text-effect-generator-1031.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/thunder.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/thunder.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'magma ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-magma-hot-text-effect-online-1030.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/magma.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/magma.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'stone ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-stone-cracked-cool-text-effect-1029.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/stone.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/stone.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'glitch ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    var topText, bottomText; 
    if (match[1].includes(';')) {
        var split = match[1].split(';');
        topText = split[0];
        bottomText = split[1];
    } else {
        topText = match[1];
        bottomText = 'ㅤ';
    }
    qatxtpro.textpro("https://textpro.me/create-glitch-text-effect-style-tik-tok-983.html",
        [`${topText}`, `${bottomText}`]
        ).then(async (data) => { 
          try { 
              var download = async(uri, filename, callback) => {
                  await request.head(uri, async(err, res, body) => {    
                      await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                  });
              };
              await download(`${data}`, '/root/UltraMax/glitch.jpg', async() => {                          
                  await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/glitch.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
              })
          } catch(err) { 
              console.log(err)
          } 
    });
}));

Max.applyCMD({pattern: '2glitch ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    var topText, bottomText; 
    if (match[1].includes(';')) {
        var split = match[1].split(';');
        topText = split[0];
        bottomText = split[1];
    } else {
        topText = match[1];
        bottomText = 'ㅤ';
    }
    qatxtpro.textpro("https://textpro.me/create-a-glitch-text-effect-online-free-1026.html",
        [`${topText}`, `${bottomText}`]
        ).then(async (data) => { 
          try { 
              var download = async(uri, filename, callback) => {
                  await request.head(uri, async(err, res, body) => {    
                      await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                  });
              };
              await download(`${data}`, '/root/UltraMax/t2gl.jpg', async() => {                          
                  await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/t2gl.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
              })
          } catch(err) { 
              console.log(err)
          } 
    });
}));


Max.applyCMD({pattern: 'harry ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-harry-potter-text-effect-online-1025.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/harry.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/harry.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'cracked ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-embossed-text-effect-on-cracked-surface-1024.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/cracked.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/cracked.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'glass ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/broken-glass-text-effect-free-online-1023.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/glass.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/glass.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'glossy ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-3d-glossy-metal-text-effect-1019.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/glossy.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/glossy.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'beach ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-3d-realistic-text-effect-on-the-beach-online-1018.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/beach.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/beach.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'watercolour ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-free-online-watercolor-text-effect-1017.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/watercolour.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/watercolour.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'devil ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-neon-devil-wings-text-effect-online-free-1014.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/devil.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/devil.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'underwater ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-underwater-text-effect-generator-online-1013.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/underwater.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/underwater.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'bear ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/online-Supun-and-white-bear-mascot-logo-creation-1012.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/bear.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/bear.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'graffiti ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-wonderful-graffiti-art-text-effect-1011.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/1graffiti.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/1graffiti.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'xmas ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-christmas-holiday-snow-text-effect-1007.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/xmas.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/xmas.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'future ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-futuristic-technology-neon-light-text-effect-1006.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/future.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/future.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'cloud ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-cloud-text-effect-on-the-sky-online-1004.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/cloud.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/cloud.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'luxury ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-luxury-gold-text-effect-online-1003.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/luxury.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/luxury.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'leaves ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/natural-leaves-text-effect-931.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/leaves.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/leaves.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'gneon ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/green-neon-text-effect-874.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/gneon.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/gneon.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'holo ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/holographic-3d-text-effect-975.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/holo.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/holo.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


Max.applyCMD({pattern: 'sand ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/write-in-sand-summer-beach-free-online-991.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/UltraMax/sand.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/UltraMax/sand.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));




const thumb = ""