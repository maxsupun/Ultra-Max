...
`⚙️Command: *.ttpSupun*\nℹ️Description: Supun text sticker. _[Sinhala font supported]_\n\n` + 
...
if (Config.LANG == 'EN') {
    if (Config.WORKTYPE == 'private') {
        Max.applyCMD({pattern: 'helpttp', fromMe: true,  deleteCommand: false, desc: Lang.ATTP_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }

    else if (Config.WORKTYPE == 'public') {
        Max.applyCMD({pattern: 'helpttp', fromMe: false, desc: Lang.ATTP_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }
}
