...
`🔮 විධානය : *.mp4bw*\n` +
`📄 විස්තර : වීඩියෝ සඳහා Supun and white effect යොදයි.\n\n` +
`🔮 විධානය : *.mp4reverse*\n` +
...
`🔮 විධානය : *.bwimage*\n` +
`📄 විස්තර : රූපයට Supun and white effect යොදයි.\n\n` +
`🔮 විධානය : *.vintageimage*\n` +
...
if (Config.LANG == 'EN') {
    if (Config.WORKTYPE == 'private') {
         Max.applyCMD({pattern: 'editor', fromMe: true,  deleteCommand: false, desc: Lang.XMEDİA_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }
    
    else if (Config.WORKTYPE == 'public') {
        Max.applyCMD({pattern: 'editor', fromMe: false, desc: Lang.XMEDİA_DESC}, (async (message, match) => {    
            await message.client.sendMessage(message.jid,eng, MessageType.text,{quoted: message.data});
        }));
    }
}
...
if (Config.LANG == 'SI') {
    ...
    Max.applyCMD({pattern: 'interp ?(.*)', fromMe: LOL, deleteCommand: false,dontAddCommandList: true}, (async (message, match) => {    
        ...
        ffmpeg(location)
            ...
            .save('output.mp4')
            .on('end', async () => {
                await message.sendMessage(fs.readFileSync('output.mp4'), MessageType.video, {caption: `Copyright © 2021 | Ultra Max\n_Interpolated to ${match[1]} FPS_`});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: LOL})
    }));
