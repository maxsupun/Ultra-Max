...
Max.applyCMD({pattern: 'sticker$', fromMe: LOL, desc: Lang.STICKER_DESC,  deleteCommand: false}, (async (message, match) => {    
...
Max.applyCMD({pattern: 'sticvid$', fromMe: LOL, desc: Lang.ANI_STICK,  deleteCommand: false}, (async (message, match) => {
    ...
    await webp2mp4File(savedFilename).then(async (rest) => {
        await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
            const saving = data.pipe(fs.createWriteStream('/root/UltraMax/vstic.mp4')) //OI OI MEKA WENAS KRAN ROOT
            saving.on("finish", async () => {
                await message.client.sendMessage(msgid, fs.readFileSync('/root/UltraMax/vstic.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: Config.CAP, quoted: message.data , thumbnail: thumb })
                if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                if (fs.existsSync('/root/UltraMax/vstic.mp4')) fs.unlinkSync('/root/UltraMax/vstic.mp4')
            })
        })
    })
}));
